...
Game::Game()
	...
	ifpause(0), TimePerFrame(sf::seconds(1.f / 10.f))
{
...
void Game::handleInput()
{
	...
	while (window_.pollEvent(event))	// 轮询处理窗口事件
	{
		if(sf::Keyboard::isKeyPressed(sf::Keyboard::P))
		{
			ifpause=1-ifpause;
			if(ifpause) TimePerFrame = sf::seconds(100000000000);
			else TimePerFrame = sf::seconds(1.f / 10.f);
		}
